...
public class FullFrameRect {
    ...
    public void drawCroppedFrame(int textureId, float[] texMatrix, Size size) {
    	FloatBuffer buffer0 = mRectDrawable.getTexCoordArray();
    	int count = buffer0.capacity();
    	if (mBuffer == null)
    	{
    		float delta = (size.width - size.height * 3 / 4 ) / 2;
            ByteBuffer bb = ByteBuffer.allocateDirect(count * Drawable2d.SIZEOF_FLOAT);
	        ...
	        for (int i = 0; i < count; i++)
	        {
	            ...
	            if (i == 5 || i == 7)
	            {
	            	fl = (size.width - delta) / size.width;
	            }
	            mBuffer.put(i, fl);
	        }
    	}
