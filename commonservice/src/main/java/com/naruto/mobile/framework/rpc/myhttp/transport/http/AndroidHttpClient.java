...
public final class AndroidHttpClient implements HttpClient {
    ...
    // Default connection and socket timeout of 20,30 seconds.  Tweak to taste.
    private static final int CONNECTION_TIMEOUT = 20 * 1000;
    private static final int SOCKET_OPERATION_TIMEOUT = 30 * 1000;

    //60 seconds,不能小于请求超时时间
    private static final int CONNECTION_POOL_TIMEOUT = 60 * 1000;
...
//            .getHttpSocketFactory(SOCKET_OPERATION_TIMEOUT, sessionCache), 443));
    ...
    private AndroidHttpClient(ClientConnectionManager ccm, HttpParams params) {
        this.delegate = new DefaultHttpClient(ccm, params) {
            ...
            protected RedirectHandler createRedirectHandler() {
                RedirectHandler handler = new DefaultRedirectHandler() {
                    /**
                     * 302跳转次数
                     */
                    int mRedirects;
    ...
    public void setHttpRequestRetryHandler(HttpRequestRetryHandler retryHandler) {
        DefaultHttpClient defaultHttpClient = (DefaultHttpClient) this.delegate;
    ...
    private static String toCurl(HttpUriRequest request, boolean logAuthToken) throws IOException {
        ...
        for (Header header : request.getAllHeaders()) {
            if (!logAuthToken
                && (header.getName().equals("Authorization") || header.getName().equals("Cookie"))) {
                continue;
            }
