package com.naruto.mobile.mvpFramework.presentermanager;
...
/**
 * A internal class responsible to save internal presenter instances during screen orientation changes and reattach the
 * presenter afterwards.
 *
 * <p> The idea is that each MVP View (like a Activity, Fragment, ViewGroup) will get a unique view id. This view id is
 * used to store the presenter and viewstate in it. After screen orientation changes we can reuse the presenter and
 * viewstate by querying for the given view id (must be saved in view's state somehow). </p>
 *
 * @author xuxinming
 * @since 3.0
 */
final public class PresenterManager {
    ...
    /**
     * Get the presenter for the View with the given (Mosby - internal) view Id or <code>null</code> if no presenter for
     * the given view (via view id) exists.
     *
    ...
    /**
     * Get the ViewState (see mosby viestate modlue) for the View with the given (Mosby - internal) view Id or
     * <code>null</code> if no viewstate for the given view exists.
     *
     * @param activity The Activity (used for scoping)
     * @param viewId   The mosby internal View Id (unique among all {@link MvpView}
    ...
    /**
     * Removes the Presenter (and ViewState) for the given View. Does nothing if no Presenter is stored internally with
     * the given viewId
     *
     * @param activity The activity
