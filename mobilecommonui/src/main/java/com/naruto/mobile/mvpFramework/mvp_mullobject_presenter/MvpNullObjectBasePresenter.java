package com.naruto.mobile.mvpFramework.mvp_mullobject_presenter;
 ...
 * object pattern</a> for the attached mvp view. So whenever the view gets detached from this presenter by calling{@link
 * #detachView(boolean)}, a new "null object" view gets dynamically instantiated by using reflections and set as the
 * current view (instead of null). The new "null object" view simply does nothing. This avoids NullPointerExceptions and
 * checks like {@code if (getView() != null)}
 *
 * <p> Please note that when creating the "null object" the first generic parameter (left depth-first search) that will
 * be found that is subtype of {@link MvpView} will be used as the type of the view. So avoid having multiple generic
 * parameters for "View" like this {@code MyPresenter<FooMvpView, OtherMvpView>} because we can't know wheter FooMvpView
 * or OtherMvpView is the real type of this presenter's view. In that case (left depth-first search) FooMvpView will be
 * used (may cause ClassCastException if OtherMvpView was the desired one) </p>
 *
