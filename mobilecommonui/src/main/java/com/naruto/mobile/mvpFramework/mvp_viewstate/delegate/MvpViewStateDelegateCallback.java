 ...
 * implemented by all (subclasses of) Activity and Fragment that want to support {@link ViewState} and mvp.
 *
 * @author xuxinming
...
public interface MvpViewStateDelegateCallback<V extends MvpView, P extends MvpPresenter<V>, VS extends ViewState<V>>
    ...
    /**
     * This method will be called by {@link MvpViewStateInternalDelegate} to inform that restoring the view state is in
     * progress.
     *
    ...
    /**
     * Called if the {@link ViewState} instance has been restored successfully. <p> In this method you have to restore
     * the viewstate by reading the view state properties and setup the view to be on the same state as before. </p>
     *
     * @param instanceStateRetained true, if the viewstate has been retained by using{@link Fragment#setRetainInstance(boolean)},
     *                              otherwise false (always false for activities).
     */
    void onViewStateInstanceRestored(boolean instanceStateRetained);

    /**
     * Called if a new {@link ViewState} has been created because no viewstate from a previous Activity or Fragment
     * instance could be restored. <p><b>Typically this is called on the first time the <i>Activity</i> or
     * <i>Fragment</i> starts and therefore no view state instance previously exists</b></p>
     */
