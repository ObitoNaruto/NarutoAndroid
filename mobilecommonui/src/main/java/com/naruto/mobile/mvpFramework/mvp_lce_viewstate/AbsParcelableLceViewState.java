...
/**
 * Extends {@link AbsLceViewState} by implementing {@link RestorableViewState}. This class can be saved and restored in
 * a bundle. Therefore it can be used for Activities and Fragments.
 *
 * <p>Please note, that {@link #restoreInstanceState(Bundle)}</p> will create a new copy of this view state and return
 * that one instead of the current object instance. That's fine and makes working custom view states much easier. This
 * note is just to inform you that the view state refrence attached to the view may change during restroing view state.
 *
