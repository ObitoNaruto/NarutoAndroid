...
package com.naruto.mobile.mvpFramework.mvp_lce;
...
/**
 * A {@link MvpFragment} that implements {@link MvpLceView} which gives you 3 options: <ul> <li>Display a loading view:
 * A view with <b>R.id.loadingView</b> must be specified in your inflated xml layout</li> <li>Display a error view: A
 * <b>TextView</b> with <b>R.id.errorView</b> must be declared in your inflated xml layout</li> <li>Display content
 * view: A view with <b>R.id.contentView</b> must be specified in your inflated xml layout</li> </ul>
 *
 * @param <CV> The type of the content view with the id = R.id.contentView. Can be any kind of android view widget like
 *             ListView, RecyclerView, ScrollView or a simple layout like Framelayout etc. (everything that extends from
 *             android.view.View)
 * @param <M>  The underlying data model that will be displayed with this view
 * @param <V>  The View interface that must be implemented by this view. You can use {@link MvpLceView}, but if you want
 *             to add more methods you have to provide your own view interface that extends from {@link MvpLceView}
...
public abstract class MvpLceFragment<CV extends View, M, V extends MvpLceView<M>, P extends MvpPresenter<V>>
    ...
    /**
     * Get the error message for a certain Exception that will be shown on {@link #showError(Throwable, boolean)}
     */
    protected abstract String getErrorMessage(Throwable e, boolean pullToRefresh);

    /**
     * The default behaviour is to display a toast message as light error (i.e. pull-to-refresh error). Override this
     * method if you want to display the light error in another way (like crouton).
     */
